 ...
 */
// Cache client instance to avoid recreating it
// تخزين مؤقت لنسخة العميل لتجنب إعادة إنشائها
let cachedClient = null

export function createClient() {
  // Return cached client if available
  // إرجاع العميل المخزن مؤقتاً إذا كان متاحاً
  if (cachedClient) {
    return cachedClient
  }
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !supabaseAnonKey) {
    ...
    return {
      auth: {
        ...
        getSession: () => Promise.resolve({ data: { session: null }, error: { message: 'Not configured' } }),
