 ...
 * All DB queries run in parallel. Results cached per (viewType, month, employeeId) to speed up repeated employee selection.
 */
import { unstable_cache } from 'next/cache'
import { prisma } from '@/lib/prisma'

const ATTENDANCE_DETAILS_LIMIT = 500
/** Cache stats for 45s so re-selecting the same employee/month is instant. */
...
async function getDashboardStatsUncached(options) {
...
export async function getDashboardStats(options = {}) {
  const { month = '', employeeId = '', viewType = 'currentMonth' } = options
  const cacheKey = ['dashboard-stats', viewType, String(month), String(employeeId)]
  return unstable_cache(
    () => getDashboardStatsUncached(options),
    cacheKey,
