...
export function calculatePayrollForEmployee(employee, attendance, deductions, month) {
  ...
  if (employee.hireDate) {
    ...
    // If employee started after the first day of the month
    // إذا بدأ الموظف بعد اليوم الأول من الشهر
    if (hireDateObj > monthStartObj) {
      // Calculate workable days from hireDate to end of month
      // حساب أيام العمل من تاريخ التعيين إلى نهاية الشهر
      const monthEndDate = new Date(monthEnd)
      monthEndDate.setHours(23, 59, 59, 999)
      
      // Calculate actual workable days (inclusive of hire date)
      // حساب أيام العمل الفعلية (شاملة تاريخ التعيين)
